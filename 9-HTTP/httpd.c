...
#define REMEMBER_SERVER "Server: HSX —— remember the age of innocence /0.0.1\r\n"  // Response Header of Server
#define STDIN   0
#define STDOUT  1
#define STDERR  2
...
void headers(int client, const char *filename, char *content_type)
{
    ...
    send(client, buf, strlen(buf), 0);
    // 响应首部字段
    strcpy(buf, REMEMBER_SERVER);           // Server: h.g. & you remember the age of innocence /0.0.1
    send(client, buf, strlen(buf), 0);
    // sprintf(buf, "Content-Type: text/html; charset=utf-8\r\n");     // application/javascript; charset=utf-8
    send(client, content_type, strlen(content_type), 0);
    printf("%s\n", content_type);
    sprintf(buf, "Date: %s hsx \r\n", ctime(&t));
    send(client, buf, strlen(buf), 0);
    sprintf(buf, "Access-Control-Allow-Headers:X-Requested-With\r\n");
    send(client, buf, strlen(buf), 0);
    sprintf(buf, "Expires: %s hsx\r\n", ctime(&t));
    send(client, buf, strlen(buf), 0);
    sprintf(buf, "Last-Modified: %s hsx\r\n", ctime(&t));
    send(client, buf, strlen(buf), 0);
